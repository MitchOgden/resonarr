# Roadmap (Working)

## Phase 1: Product/architecture foundation

- Lock canonical entity schema
- Lock action-intent schema
- Lock first provider choice
- Lock MVP feedback signals
- Lock first scoring model dimensions
- Lock monitoring presets
- Lock automation modes

## Phase 2: MVP execution design

- Define provider connector boundary
- Define normalization and identity resolution flow
- Define policy engine inputs/outputs
- Define Lidarr execution adapter behavior
- Define memory/suppression behavior
- Define dry-run and audit model

## Phase 3: MVP operator experience

- Define minimum UI surface
- Define dry-run views
- Define action history / audit views
- Define suppression management view
- Define config/setup flow

## Phase 4: Implementation planning

- Convert planning artifacts into implementation-facing spec
- Break into backend modules and data model
- Define API surface
- Define job/scheduler approach
- Define deployment shape
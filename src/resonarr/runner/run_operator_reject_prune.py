...
def main():
    configure_runner_logging("operator-reject-prune")
    service = PruneOperatorService()

    if len(sys.argv) < 3:
        print('Usage: python -m resonarr.runner.run_operator_reject_prune "Artist Name" "Album Name"')
        sys.exit(1)

    artist_name = sys.argv[1]
    album_name = sys.argv[2]

    print("=== Resonarr Reject Prune Recommendation ===")
    print(f"[INFO] Artist: {artist_name}")
    print(f"[INFO] Album: {album_name}")

    result = service.reject_review_item(artist_name, album_name)
